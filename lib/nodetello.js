	...
	*/
	...
	this._sendCmd = function(pacType, cmdID, data, data_len) {
		...
		if (cmdID == this.tello.commands.TELLO_CMD_CONN) {
			...
			bb = Buffer.alloc(11)
			var tmp_roll     = parseInt(1024 + 660.0 * this.stickData.roll) & 0x7ff;
			var tmp_pitch    = parseInt(1024 + 660.0 * this.stickData.pitch) & 0x7ff;
			var tmp_throttle = parseInt(1024 + 660.0 * this.stickData.throttle) & 0x7ff;
			var tmp_yaw      = parseInt(1024 + 660.0 * this.stickData.yaw) & 0x7ff;
			
			bb[0]=((tmp_pitch << 11 | tmp_roll) >> 0) & 0xff;
			bb[1]=((tmp_pitch << 11 | tmp_roll) >> 8) & 0xff;
			bb[2]=((tmp_throttle << 11 | tmp_pitch) >> 5) & 0xff;
			bb[3]=((tmp_yaw << 11 | tmp_throttle) >> 2) & 0xff;
			bb[4]=((tmp_yaw << 11 | tmp_throttle) >> 10) & 0xff;
			bb[5]=((tmp_yaw << 11 | tmp_throttle) >> 18) & 0xff;
		...
		*/
		console.log("TELLO_CMD:", data);
			...
			if (data.cmd.cmd == 'takeoff') {
				...
				// Should be 0xcc, 0x58, 0x00, 0x7c, 0x68, 0x54, 0x00, 0xe4, 0x01, 0xc2, 0x16
				// my          cc    58    00    7c    68    54    00    7b    02    cc    be
				// my          cc    58    00    7c    68    54    00    f5    00    02    8b
				// py        0xcc, 0x58, 0x00, 0x7c, 0x68, 0x54, 0x00, 0x09, 0x00, 0xaa, 0x5e	// WTF?
				// my9         cc    58    00    7c    68    54    00    09    00    aa    5e
