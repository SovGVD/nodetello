...
var nodetello = function () {
		...
		if (bb.length >= 11) {
			...
			if (mark == 0xCC) {	// start of packet
				...
				if (cmdID == this.tello.commands.TELLO_CMD_WIFI_SIGNAL) {
					...
					console.log("DBG", "unknow CMD", cmdID);
				}
			} else {
				if (mark == 0x63) {
	...
	this._sendCmd = function(pacType, cmdID, data, data_len) {
		...
		if (cmdID == this.tello.commands.TELLO_CMD_CONN) {
			...
			this.seqID++
		/*} else if (cmdID == this.tello.commands.TELLO_CMD_LOG_HEADER_WRITE) {
			seq = parseInt(this.seqID)
			bb = Buffer.alloc(1)
			bb.writeUInt8(0x00,0)
			this.seqID++*/
		...
		while (logPosition < (size-2)) {
			...
			if (typeof this.tello.log.LOGREC[recSpecId] != 'undefined') {
				...
				for (var i in this.tello.log.LOGREC[recSpecId].fields) { if (typeof this.tello.log.LOGREC[recSpecId].fields[i] == 'object') {
					var o = this.tello.log.LOGREC[recSpecId].fields[i];
					if (typeof this.status.LOG[(this.tello.log.LOGREC[recSpecId].NAME)] == 'undefined') {
						this.status.LOG[(this.tello.log.LOGREC[recSpecId].NAME)]={};
					}
					switch (o.type) {
						case this.TYPE.FLOAT:
							this.status.LOG[(this.tello.log.LOGREC[recSpecId].NAME)][i] = xorBuf.readFloatLE(baseOffset + o.offset);
							break;
						case this.TYPE.SHORT:
							this.status.LOG[(this.tello.log.LOGREC[recSpecId].NAME)][i] = xorBuf.readInt16LE(baseOffset + o.offset);
							break;
						case this.TYPE.BYTE:
							this.status.LOG[(this.tello.log.LOGREC[recSpecId].NAME)][i] = xorBuf[baseOffset + o.offset];
							break;
						default:
							console.log("DBG", "unknown type", o.type);
					}
				}}
				...
				console.log("LOG:", this.status.LOG);
			} else {
	...
	this.QtoDeg = function () {
		// https://github.com/SMerrony/tello/blob/master/flog.go
		try {
		var sqX = Math.pow(this.status.LOG.IMU.Q_X,2);
		var sqY = Math.pow(this.status.LOG.IMU.Q_Y,2);
		var sqZ = Math.pow(this.status.LOG.IMU.Q_Z,2);
		
		var sinR = 2 * (this.status.LOG.IMU.Q_W * this.status.LOG.IMU.Q_X + this.status.LOG.IMU.Q_Y * this.status.LOG.IMU.Q_Z);
		var cosR = 1 - 2 * (sqX + sqY);
		this.status.LOG.IMU.ANGLE_ROLL = parseFloat(Math.atan2(sinR, cosR) / this.DEG);
		
		var sinP = 2 * (this.status.LOG.IMU.Q_W * this.status.LOG.IMU.Q_Y - this.status.LOG.IMU.Q_Z * this.status.LOG.IMU.Q_X);
		if (sinP > 1) {
			sinP = 1;
		} else if (sinP < -1) {
			sinP = -1;
		}
		this.status.LOG.IMU.ANGLE_PITCH = parseFloat(Math.asin(sinP) / this.DEG);
		
		var sinY = 2 * (this.status.LOG.IMU.Q_W * this.status.LOG.IMU.Q_Z + this.status.LOG.IMU.Q_X * this.status.LOG.IMU.Q_Y);
		var cosY = 1 - 2 * (sqY + sqZ);
		this.status.LOG.IMU.ANGLE_YAW = parseFloat(Math.atan2(sinY, cosY) / this.DEG);
		}  catch (e) {
