	...
	*/
	...
	this._sendCmd = function(pacType, cmdID, data, data_len) {
		...
		if (cmdID == this.tello.commands.TELLO_CMD_CONN) {
			...
			// TODO... if (tmp_roll.length == 10) tmp_roll="0"+tmp_roll; = TEMPORARY for very short delta!!!!
			var tmp_roll     = (parseInt(1024 + 660 * this.stickData.roll) & 0x07ff).toString(2); if (tmp_roll.length == 10) tmp_roll="0"+tmp_roll;
			var tmp_pitch    = (parseInt(1024 + 660 * this.stickData.pitch) & 0x07ff).toString(2); if (tmp_pitch.length == 10) tmp_pitch="0"+tmp_pitch;
			var tmp_throttle = (parseInt(1024 + 660 * this.stickData.throttle) & 0x07ff).toString(2); if (tmp_throttle.length == 10) tmp_throttle="0"+tmp_throttle;
			var tmp_yaw      = (parseInt(1024 + 660 * this.stickData.yaw) & 0x07ff).toString(2); if (tmp_yaw.length == 10) tmp_yaw="0"+tmp_yaw;
			var tmp_boost    = (parseInt(1024 + 660 * 0)).toString(2);	// 0 - slow, 1 - fast???
			
			// well... JS is 32 bit only and for this thing we need 64bit (11*4 bit + 4 more for 48bit value)... lets do this as strings
			// !!!!string should be 11bits (chars)!!!!
			var rc = "0000"+tmp_yaw.toString(2)+tmp_throttle.toString(2)+tmp_pitch.toString(2)+tmp_roll.toString(2);
			bb.writeUInt8(parseInt(rc.substr(-8),2), 0);
			...
			console.log("DBG",  [tmp_roll.length, tmp_pitch.length, tmp_throttle.length, tmp_yaw.length, tmp_boost.length], rc, rc.length, bb);
			console.log("DBG", rc, '=>', rc.substr(-48,8), rc.substr(-40,8), rc.substr(-32,8), rc.substr(-24,8), rc.substr(-16,8), rc.substr(-8,8));
			// 00 04 20 00 01 08 12 1f 1b 58 5c
			//                                                          9    10    11   12     13    14
			// 0xcc, 0xb0, 0x00, 0x7f, 0x60, 0x50, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0c, 0x0b, 0x16, 0x81, 0xd1, 0x4b, 0xf1
			seq = 0
		...
		*/
		//console.log("TELLO_CMD:", data);
			...
			if (data.cmd.cmd == 'stick') {
				// TODO check values limits!!!
				this.stickData.roll = data.cmd.value.roll;
				this.stickData.pitch = data.cmd.value.pitch;
				this.stickData.throttle = data.cmd.value.throttle;
				this.stickData.yaw = data.cmd.value.yaw;
				console.log("Stick", data.cmd.value);
			}
